...
zip_mod <- stan_jsdm(dat_list = data_sim, family = "zi_poisson",
                     method = "mglmm", cores = 4, iter = 2000,
                     prior = jsdm_prior(betas = "normal(1,0.5)",
                                        zi_betas = "normal(0.5,0.25)"))
```

This model runs without any warnings, and if we print the model summary we can see that there are no problematic parameters:
...
We can see that the model is massively overestimating the number of zeroes for each species, but is getting the mean correct within the non-zero records.
...
From this we can see that the model is overestimating the contribution of the zero-inflation to the model output. 
...
This is matched by overestimating the intercepts of the poisson part of the model - the data can be fit by assuming zeroes come from either the zero-inflation part or the poisson part, and this model really 
