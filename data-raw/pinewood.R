...
site_locs <- read.csv(paste0(data.dir,"Scots_Pine_1971_Sites.csv"))
str(site_locs)
...
# soil vars
soils <- read.csv(paste0(data.dir,"PINEWOODS1971_SOIL_DATA.csv"))
str(soils)
...
# tree data
trees <- read.csv(paste0(data.dir,"PINEWOODS1971_TREE_DATA.csv"))
str(trees)
...
site_env_tree <- site_env %>%
  select(Site_no:Soil_PH, SitePlot) %>%
  filter(!is.na(Soil_PH) & SitePlot %in% tree_wide$SitePlot) %>%
  select(Site_no, Plot_no, SitePlot, everything()) %>%
  arrange(SitePlot)

tree_site <- tree_wide %>%
  filter(SitePlot %in% site_env_tree$SitePlot) %>%
  arrange(SitePlot) %>%
  tibble::column_to_rownames("SitePlot") %>%
  select(Pinus_sylvestris:Fraxinus_sp)
...
#
# flora <- read.csv(paste0(data.dir,"PINEWOODS1971_GROUND_FLORA.csv"))
# str(flora)
...
# pinewood <- pinewood[site_env$SitePlot,]
...
#   group_by(Site_no, OSGR) %>% summarise(across(.fns = mean, na.rm = TRUE),
...
#   filter(No_plots == 0) %>%
#   select(-No_plots)
# pinewood <- pinewood[as.character(pinewood_env$Site_no),]
#
# usethis::use_data(pinewood, overwrite = TRUE)
# usethis::use_data(pinewood_env, overwrite = TRUE)
