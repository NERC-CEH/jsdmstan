...
test_that("stan_mglmm returns right type of object", {
  ...
  mglmm_fit <- stan_mglmm(dat_list = mglmm_data, family = "bern",
                          refresh = 0, iter = 500)

  expect_s3_class(mglmm_fit, "jsdmStanFit")

  # gaussian
  mglmm_data <- mglmm_sim_data(N = 50, S = 3, family = "gauss",
                               site_intercept = TRUE)
  mglmm_fit <- stan_mglmm(dat_list = mglmm_data, family = "gauss",
                          refresh = 0, iter = 500, site_intercept = TRUE)

  expect_s3_class(mglmm_fit, "jsdmStanFit")

  # poisson
  mglmm_data <- mglmm_sim_data(N = 200, S = 8, K = 3, family = "poisson")
  mglmm_fit <- stan_mglmm(dat_list = mglmm_data, family = "poisson",
                          refresh = 0, iter = 500)
  ...
  mglmm_fit <- stan_mglmm(dat_list = mglmm_data, family = "neg_bin",
                          refresh = 0, iter = 500)
