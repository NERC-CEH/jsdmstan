...
test_that("phylo models error appropriately", {
  expect_error(
    mglmm_sim_data(N = 500, S = 12, phylo = TRUE, family = "poisson"),
    "Need to specify delta and covar arguments for phylo"
  )
  phylo_data <- mglmm_sim_data(
    N = 500, S = 12, phylo = TRUE, family = "poisson",
    delta = 1e-5, covar = "matern_05"
  )
  expect_error(
    stan_jsdm(
      X = phylo_data$X, Y = phylo_data$Y, method = "gllvm",
  ...
  expect_error(
    stan_jsdm(
      X = phylo_data$X, Y = phylo_data$Y, method = "mglmm",
      family = "poisson", phylo = TRUE
    ),
...
test_that("phylo poisson matern 25 models run", {
  set.seed(46153123)
  phylo_data <- mglmm_sim_data(
    N = 123, S = 12, K = 3, phylo = TRUE,
    family = "poisson",
    delta = 1e-5, covar = "matern_25"
  )
  expect_named(phylo_data, c(
    "Y", "pars", "N", "S", "D", "K", "X",
...
test_that("phylo sq exponential bern models run", {
  set.seed(46153123)
  phylo_data2 <- jsdm_sim_data(
    N = 100, S = 10, phylo = TRUE, family = "bern",
    delta = 1e-4, covar = "sq_exponential",
    method = "mglmm"
  )
  expect_named(phylo_data2, c(
    "Y", "pars", "N", "S", "D", "K", "X",
    "site_intercept", "Dmat", "delta", "nu05"
  ))
  suppressWarnings(phylo_fit2 <- stan_jsdm(
    X = NULL, dat_list = phylo_data2,
    method = "mglmm",
    family = "bern",
...
test_that("phylo gaussian exponential models run", {
  phylo_data3 <- jsdm_sim_data(
    N = 100, S = 10, phylo = TRUE, family = "gaussian",
    delta = 1e-5, covar = "exponential",
    method = "mglmm"
  )
  expect_named(phylo_data3, c(
    "Y", "pars", "N", "S", "D", "K", "X",
    "site_intercept", "Dmat", "delta", "nu05"
  ))
  suppressWarnings(phylo_fit3 <- stan_jsdm(
    X = NULL, dat_list = phylo_data3,
    method = "mglmm",
    family = "gaussian",
...
test_that("phylo negbin matern 15 models run", {
  phylo_data4 <- jsdm_sim_data(
    N = 100, S = 10, phylo = TRUE,
    family = "neg_binomial",
    delta = 1e-5, covar = "matern_15",
    method = "mglmm"
  )
  expect_named(phylo_data4, c(
    "Y", "pars", "N", "S", "D", "K", "X",
    "site_intercept", "Dmat", "delta", "nu05"
  ))
  suppressWarnings(phylo_fit4 <- stan_jsdm(
    X = NULL, dat_list = phylo_data4,
    method = "mglmm",
    family = "neg_binomial",
